...
//           system("espeak \"Mission resuming.\" -ven+f4 -s130 -a40 2>/dev/null &");
        ...
        switch(mission)
        {
          case 1: // running auto mission
            ended = missionGolfballPlatform(missionState);
            break;
          default:
...
bool UMission::missionGolfballPlatform(int& state)
{
  bool finished = false;
  switch (state)
  {
    case 0:
      std::cout << "# executing mission 4, golfball platform\n";
      state = 10;
      break;
    case 10:
      snprintf(lines[0], MAX_LEN, "tr=0, vel=0.1 : turn = -95");
      snprintf(lines[1], MAX_LEN, "vel=0.15, acc=0.5 : lv >=3");
      snprintf(lines[2], MAX_LEN, "edgel=0 : event=4");
      sendAndActivateSnippet(lines, 3);
      state++;
      break;
    case 11:
      if(bridge->line->lineValues[7]>=2500)
      {
        bridge->event->setEvent(4);
        state++;
      } 
      break;
    case 12:
      if(bridge->event->isEventSet(4))
      {
        ...
        //that's from the junction
        //snprintf(lines[line++], MAX_LEN, "tr=0, vel=0.1 : turn=-65");

        //drive towards the ball
        snprintf(lines[line++], MAX_LEN, "vel=0.0, edgel=0, white = 1 : time = 1");
        snprintf(lines[line++], MAX_LEN, "vel=0.15 : dist= 0.3");
        snprintf(lines[line++], MAX_LEN, "vel=0.15, acc=0.5, edgel=0, white = 1 : tilt < 2.8");
        snprintf(lines[line++], MAX_LEN, "vel = 0.2, tr=0.0: turn=-180.0");
        snprintf(lines[line++], MAX_LEN, "vel=0.1, edgel=0, white=1: dist = 0.2");
        snprintf(lines[line++], MAX_LEN, "tr = 0: turn = 95");
        snprintf(lines[line++], MAX_LEN, "vel=0.1: dist = 0.07");
        ...
        //ball is picked up, drive towards the hole
        snprintf(lines[line++], MAX_LEN, "vel = 0.1, tr=0 : turn=-140");
        //snprintf(lines[line++], MAX_LEN, "vel=0.3, acc=0.5 : dist=0.6");
        
        //hole search pattern loop
        snprintf(lines[line++], MAX_LEN, "label=50, vel=0.1, acc=1.0 : dist = 0.04");
        snprintf(lines[line++], MAX_LEN, "tr=0.0: turn=7.0");
        snprintf(lines[line++], MAX_LEN, "tr=0.0 : turn=-14.0");
        snprintf(lines[line++], MAX_LEN, "tr=0.0 : turn=7.0");
        snprintf(lines[line++], MAX_LEN, "goto=50 : count=4");

        //stop mission
        snprintf(lines[line++], MAX_LEN, "vel=0.0, event=7");
      ...
      case 13:
        ...
        {
          finished = true;
          printf("Golfball platform mission has finished");
        }
      break;
  }
  return finished;
