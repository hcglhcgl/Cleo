...
//           system("espeak \"Mission resuming.\" -ven+f4 -s130 -a40 2>/dev/null &");
        ...
        switch(mission)
        {
          ...
          case 4:
            ...
            break;*/
          case 1: // running only stairs mission
...
bool UMission::mission_stairs(int & state)
{
  ...
  {
    case 0:
      printf("Started stairs mission.\n");
     // play.say("Jacopo you look very beautiful today.", 100);
      bridge->send("oled 5 stairs start");
      state++;
      break;
    case 1:
    {
      ...
      // go backwards and find the line
      snprintf(lines[line++], MAX_LEN, "servo=2, pservo=-250, vservo=0");
      snprintf(lines[line++], MAX_LEN, "servo=3, pservo=235, vservo=0");
      snprintf(lines[line++], MAX_LEN, "vel=-0.12: lv=4, dist=0.5");

      // go pass the crossing and then turn 180deg
      snprintf(lines[line++], MAX_LEN, "vel = 0.1, edgel=0.0, white=1: dist = 0.1");
      snprintf(lines[line++], MAX_LEN, "vel = 0.4, edgel=0, white=1: tilt < 2.9");
      snprintf(lines[line++], MAX_LEN, "vel = 0.2, tr=0.0: turn=-180.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.1, edgel=0, white=1: dist = 0.9");

      // lower the arm
      snprintf(lines[line++], MAX_LEN, "label=1");
      snprintf(lines[line++], MAX_LEN, "servo=2, pservo=700, vservo=0");
      snprintf(lines[line++], MAX_LEN, "servo=3, pservo=-685, vservo=0");
      snprintf(lines[line++], MAX_LEN, "vel=0.0: time=1");
      snprintf(lines[line++], MAX_LEN, "vel=0.2 : dist=0.25");
      

      // repeat for 4 steps
      snprintf(lines[line++], MAX_LEN, "servo=2, pservo=-250, vservo=0");
      snprintf(lines[line++], MAX_LEN, "servo=3, pservo=235, vservo=0");
      snprintf(lines[line++], MAX_LEN, "vel=-0.2 : time=1.0");
      snprintf(lines[line++], MAX_LEN, "vel = 0.0, edger=2, white=1 : time=2.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.2 , edger=2, white=1: dist=0.17");
      snprintf(lines[line++], MAX_LEN, "goto=1 : count=4");
      
      /*
      snprintf(lines[line++], MAX_LEN, "vel=0.15, edgel=-2, white=1: ");
      snprintf(lines[line++], MAX_LEN, "vel = 0.0 : time = 2.0");
      snprintf(lines[line++], MAX_LEN, "vel=0.15 : tilt< -2.8");
      snprintf(lines[line++], MAX_LEN, "vel=-0.1: time=1");
      snprintf(lines[line++], MAX_LEN, "vel=0.0, edgel=-2: time=2.5");
      snprintf(lines[line++], MAX_LEN, "goto=1: count=4");
      */
    ...
    case 2:
      ...
      if (bridge->event->isEventSet(1))
      {
        printf("Case 2 - sending commands.\n");
        int line = 0;
        // last step
        // snprintf(lines[line++], MAX_LEN, "vel=0.1, edgel=0.0, white=1: tilt>-2.0");
        //snprintf(lines[line++], MAX_LEN, "vel = 0.0 : time = 0.5");
        //snprintf(lines[line++], MAX_LEN, "vel=0.1, edgel=0.0, white=1: tilt< -2.0");

        // bring arm to the default pos, follow the line for 70cm
        snprintf(lines[line++], MAX_LEN, "servo=2, pservo=-250, vservo=0");
        snprintf(lines[line++], MAX_LEN, "servo=3, pservo=235, vservo=0");
        snprintf(lines[line++], MAX_LEN, "vel=0.4, edgel=0.0, white=1: dist=0.7");

        // stop the robot and set the event to 5
        snprintf(lines[line++], MAX_LEN, "vel=0.0, event=5");
        sendAndActivateSnippet(lines, line);
        state++;
      }
      ...
      break;
    case 3:
      printf("Case 2 - waiting for event=5.\n");
      if (bridge->event->isEventSet(5))
      { // all lines were executed
        printf("Case 3 - setting state=999.\n");
        state = 999;
      }
      break;
    case 999:
    default:
      printf("Ended stairs mission.\n");
      bridge->send("oled 5 \"stairs end\"");
      finished = true;
      break;
  }
  return finished;
