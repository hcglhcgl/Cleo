...
//           system("espeak \"Mission resuming.\" -ven+f4 -s130 -a40 2>/dev/null &");
        ...
        switch(mission)
        {
          case 1: // running auto mission
            ended = mission_seesaw_part1(missionState);
            break;
          case 2: // running auto mission
            ended = mission_seesaw_part2(missionState);
            break;
          case 3: // running auto mission
            ended = mission_seesaw_part3(missionState);
            break;                      
          case 4:
...
bool UMission::mission_seesaw_part1(int & state)
{
  bool finished = false;
...
//       system("espeak \"press green to start\" -ven+f4 -s130 -a5 2>/dev/null &");
    ...
    case 1:
      if (bridge->joy->button[BUTTON_GREEN])
        state = 2;
      break;
    case 2: // first PART - wait for IR2 then go fwd and turn
    
      snprintf(lines[0], MAX_LEN, "thread =2, event = 1: event= 5");
      snprintf(lines[1], MAX_LEN, "servo=2, pservo=-230, vservo=0 : event = 2");
      snprintf(lines[2], MAX_LEN, "servo=2, pservo=230, vservo=0 : event = 3");
      snprintf(lines[3], MAX_LEN, "servo=2, pservo=330, vservo=0 : event = 4");
      snprintf(lines[4], MAX_LEN, "thread =2, event = 1: event= 5");
      snprintf(lines[5], MAX_LEN, "servo=3, pservo=215, vservo=0 : event = 2");
      snprintf(lines[6], MAX_LEN, "servo=3, pservo=-215, vservo=0 : event = 3");
      snprintf(lines[7], MAX_LEN, "servo=3, pservo=-315, vservo=0 : event = 4");
      // This drives the robot off the seesaw and turns it to the right

      snprintf(lines[8], MAX_LEN, "thread = 1");
      snprintf(lines[9], MAX_LEN, "event = 1");
      snprintf(lines[10], MAX_LEN, "vel=0.1 : dist = 0.1");
      snprintf(lines[11], MAX_LEN, "vel=0.3, edger=0.0, white=1 : ir1<0.2");
      snprintf(lines[12], MAX_LEN, "vel=0.35, edger=0.0, white=1: dist=0.65");
      snprintf(lines[13], MAX_LEN, "vel=0.0: time = 0.5");
      snprintf(lines[14], MAX_LEN, "tr=0.0, vel=0.1: turn=80.0");
      snprintf(lines[15], MAX_LEN, "vel=0.3, edger=0.0, white=1 : dist=1.3");
      snprintf(lines[16], MAX_LEN, "vel = 0.0, event = 10");
    ...
    case 999:
    default:
      printf("mission seesaw part 1 ended \n");
      bridge->send("oled 5 \"mission seesaw part 1 ended.\"");
      finished = true;
      break;
  }
  return finished;
...
bool UMission::mission_seesaw_part2(int & state)
{
  bool finished = false;
  // First commands to send to robobot in given mission
  // (robot sends event 1 after driving 1 meter)):
  switch (state)
  {
    ...
    case 2: // first PART - wait for IR2 then go fwd and turn
      snprintf(lines[0], MAX_LEN, "thread =2, event = 1: event= 5");
      snprintf(lines[1], MAX_LEN, "servo=2, pservo=250, vservo=0 : event = 3");
      snprintf(lines[2], MAX_LEN, "servo=2, pservo=330, vservo=0 : event = 4");
      snprintf(lines[3], MAX_LEN, "servo=2, pservo=250, vservo=0 : event = 6");
      snprintf(lines[4], MAX_LEN, "thread =3, event = 2: event= 5");
      snprintf(lines[5], MAX_LEN, "servo=3, pservo=-235, vservo=0 : event = 3");
      snprintf(lines[6], MAX_LEN, "servo=3, pservo=-315, vservo=0 : event = 4");
      snprintf(lines[7], MAX_LEN, "servo=3, pservo=-235, vservo=0 : event = 6");
      // This drives the robot off the seesaw and turns it to the right
    
      snprintf(lines[8], MAX_LEN, "vel=0.0: time = 1");
      snprintf(lines[9], MAX_LEN, "vel=0.0,event =2: time = 1");
      snprintf(lines[10], MAX_LEN, "vel=0.3, edger=0.0, white=1: ir1<0.2");
      snprintf(lines[11], MAX_LEN, "thread = 1");
      snprintf(lines[12], MAX_LEN, "event = 2");
      snprintf(lines[13], MAX_LEN, "vel=0.3, acc=1.0: dist=0.85");
      snprintf(lines[14], MAX_LEN, "tr=0.0: turn=-95.0");
      snprintf(lines[15], MAX_LEN, "vel=0.3, acc=1.0, white=1: xl>12");
      snprintf(lines[16], MAX_LEN, "vel=0.3, acc=1.0: dist=0.1");
      snprintf(lines[17], MAX_LEN, "vel=0.3, acc=1.0, white=1: xl>12");
      snprintf(lines[18], MAX_LEN, "vel=0.0, event = 11");
      sendAndActivateSnippet(lines, 19);
    ...
    case 999:
    default:
      printf("mission seesaw part 2 ended \n");
      bridge->send("oled 5 \"mission seesaw part 2 ended.\"");
      finished = true;
      break;
  }
  return finished;
...
bool UMission::mission_seesaw_part3(int & state)
{
  bool finished = false;
  // First commands to send to robobot in given mission
  // (robot sends event 1 after driving 1 meter)):
  switch (state)
  {
    case 0:
      ...
      break;
    case 2: 
      snprintf(lines[0], MAX_LEN, "tr=0.0: turn=-90.0");
      snprintf(lines[1], MAX_LEN, "vel=0.3, edgel=0.0, white=1: dist=0.7");
      snprintf(lines[2], MAX_LEN, "event=3");
      snprintf(lines[3], MAX_LEN, "vel=0.3, edgel=0.0, white=1: tilt>3.0");
      snprintf(lines[4], MAX_LEN, "event = 4");
      snprintf(lines[5], MAX_LEN, "tr=0.0: turn=5.0");
      snprintf(lines[6], MAX_LEN, "vel=0.1, acc=1.0: dist=0.15");
      
      snprintf(lines[7], MAX_LEN, "label=50, vel=0.1, acc=1.0: dist=0.04");
      snprintf(lines[8], MAX_LEN, "tr=0.0: turn=7.0");
      snprintf(lines[9], MAX_LEN, "tr=0.0: turn=-14.0");
      snprintf(lines[10], MAX_LEN, "tr=0.0: turn=7.0");
      snprintf(lines[11], MAX_LEN, "goto=50 : count=4");
      snprintf(lines[12], MAX_LEN, "vel=0.0, event = 12");      
      sendAndActivateSnippet(lines, 13);
      
      if (bridge->event->isEventSet(12)) //Dont know what this value does.
      { // 
        state = 999;
      }
      break;
    ...
    case 999:
    default:
      printf("mission seesaw part 3 ended \n");
      bridge->send("oled 5 \"mission seesaw part 3 ended.\"");
      finished = true;
      break;
  }
  return finished;
