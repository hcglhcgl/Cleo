...
{ /// current mission number
  mission = fromMission;
  //mission = 1; // is this necessary?
  int missionOld = fromMission;
  ...
  //           system("espeak \"Mission resuming.\" -ven+f4 -s130 -a40 2>/dev/null &");
        ...
        {
          case 1: 
            ended = missionGuillotineRamp(missionState);
            break;
          /*
...
/**
 * Run mission Guillotine + 1st ramp
 * \param state is kept by caller, but is changed here
 *              therefore defined as reference with the '&'.
 *              State will be 0 at first call.
 * \returns true, when finished. */
bool UMission::missionGuillotineRamp(int & state)
{
  bool finished = false;
  switch (state)
  {
    ...
    case 1:
      ...
      snprintf(lines[0], MAX_LEN, "servo=2, pservo=-800, vservo=0");
      snprintf(lines[1], MAX_LEN, "servo=3, pservo=790, vservo=0");
      //printf("Case 10 started, robot should start moving!\n");
      snprintf(lines[2], MAX_LEN, "vel=0.0, edger=3, white=1: time=2");
      snprintf(lines[3], MAX_LEN, "vel=0.1, edger = 3, white = 1: dist = 0.2");
      //snprintf(lines[line++], MAX_LEN, "vel=0.1: dist = 1.1");
      snprintf(lines[4], MAX_LEN, "vel=0.35, acc=1, edger=3, white=1: ir1<0.2"); // start and approach guillotine
      snprintf(lines[5], MAX_LEN, "vel=0.55, edger=2, white=1: dist=0.7"); // pass the guillotine
      snprintf(lines[6], MAX_LEN, "vel=0.55, edger=2, white=1: ir1<0.2"); 
      //printf("Case 10 started, robot should start moving!\n");
      /*
      snprintf(lines[line++], MAX_LEN, "vel=0.5, acc=1: dist=0.25");
      //snprintf(lines[line++], MAX_LEN, "vel=0.1, edgel=0.0, white=1: dist = 0.1");
      snprintf(lines[line++], MAX_LEN, "vel=0.35, acc=1, edger=3, white=1: ir1<0.2"); // start and approach guillotine
      snprintf(lines[line++], MAX_LEN, "vel=0.55, edger=2, white=1: dist=0.7"); // pass the guillotine
      snprintf(lines[line++], MAX_LEN, "vel=0.55, edger=2, white=1: ir1<0.2"); // follow the line until you approach the first ramp
      snprintf(lines[line++], MAX_LEN, "vel=0.25, edgel=-2, white=1: xl>5");
      snprintf(lines[line++], MAX_LEN, "vel=0.35, edgel=-2, white=1: dist = 0.05"); // go up the ramp until you find crossroad
      //snprintf(lines[3], MAX_LEN, "vel=0.35, edger=0.0, white=1: dist=0.55"); // go up the ramp hardcoded
      snprintf(lines[line++], MAX_LEN, "tr=0.0: turn=90.0"); // turn in place in the crossroad
      snprintf(lines[line++], MAX_LEN, "vel=0.0, event=1"); // to signal that the mission ends4
      */
      // send the 5 lines to the REGBOT
    ...
    case 999:
     default:
      //printf("Case 999!\n");
      printf("mission Guillotine+Ramp ended \n");
      bridge->send("oled 5 \"mission Guillotine+Ramp ended.\"");
      finished = true;
      break;
  }
  return finished;
