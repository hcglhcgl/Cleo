...
{ // show help
  ...
  printf("     Use q to quit (proper shutdown of devices).\n\n");
}
 ...
 * */
int getLineFromKeyboard(char * s, const int MSL, volatile bool * fin)
{
  char * p1 = s;
  *p1 = '\0';
  while (((p1 - s) < MSL) and not *fin)
  {
...
int main ( int argc,char **argv ) 
{
  ...
  { // create connection to Regbot board through bridge 
    ...
    { // Just wait for quit
      int n = getLineFromKeyboard(s, MSL, &mission.finished);
//       const char * p1 = s;
      if (n < 0)
        break;
      if (n == 0)
